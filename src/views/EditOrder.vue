          ...
          <div class="lg:px-2 lg:w-1/2">
            ...
            <div class="p-4">
              ...
              >
                ...
                <span class="ml-2 mt-5px">Update Order</span>
              </button>
            </div>
          </div>
        </div>
...
import { update_order, order_details } from "@/api/urls.js";
export default {
  name: "EditOrder",
  data() {
    return {
  ...
  methods: {
    ...
    updateOrder() {
      ...
      self.$axios
        .post(`${update_order}/${self.$route.params.id}`, self.form)
        .then((res) => {
          this.$toastr.s("Order updated success!");
        })
        .finally((res) => {
    ...
    getOrder() {
      ...
      self.$axios
        ...
        .then((res) => {
          ...
          self.form.items = res.data.data.details;
          self.form.total_amount = res.data.data.total_amount;
          self.form.instruction = res.data.data.instruction;
        })
        .finally((res) => {
          self.$store.commit("set_is_loading", false);
        });
